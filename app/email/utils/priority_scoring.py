...
class PriorityScorer:
    ...
    def score(self, sender: str, nlp_results: Dict, llm_results: Dict) -> Tuple[int, str]:
        ...
        score = ProcessingConfig.BASE_PRIORITY_SCORE
        
        if sender in self.vip_senders:
            score += ProcessingConfig.VIP_SCORE_BOOST
        if nlp_results['is_urgent']:
            score += ProcessingConfig.URGENCY_SCORE_BOOST
        if llm_results['needs_action']:
            score += ProcessingConfig.ACTION_SCORE_BOOST
            
        score = min(score, ProcessingConfig.MAX_PRIORITY)
