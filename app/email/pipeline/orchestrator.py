...
class EmailPipeline:
    ...
    async def get_analyzed_emails_stream(self, command: AnalysisCommand) -> AsyncGenerator[dict, None]:
        ...
        try:
            ...
            # Set up user context and validate
            user_id, user_email, timezone_obj, ai_enabled, cache_duration = await self._setup_streaming_context(command)
            
            # Get cached emails and yield them to the client
            cached_emails = []
            cached_ids = set()
            
            if self.cache:
                # Send cache status message
                async for status in self._send_cache_status():
                    yield status
                
                # Get cached emails
                cached_emails, cached_ids = await self._fetch_cached_emails(command, user_email, timezone_obj, stats)
                if cached_emails:
                    yield {
                        'type': 'cached',
            ...
            # Fetch and preprocess emails from Gmail
            raw_emails, gmail_email_ids, new_raw_emails = await self._fetch_streaming_emails(
                command, user_email, timezone_obj, cached_ids, cached_emails, stats
            )
            
            # Filter cached emails and update if needed
            cached_emails, cached_ids = self._filter_cached_emails(cached_emails, gmail_email_ids, stats)
            if stats.get("deleted_emails_filtered", 0) > 0:
            ...
            # If no new emails, yield stats and return
            ...
            if not ai_enabled:
                # Process without AI
                async for result in self._process_without_ai(parsed_emails, user_email, cache_duration, stats):
                    yield result
            else:
                # Process with AI
                if command.batch_size:
                    # Process in batches
                    async for result in self._process_in_batches(parsed_emails, command, user_id, user_email, ai_enabled, cache_duration, stats):
                        yield result
                else:
                    # Process all at once
                    async for result in self._process_all_at_once(parsed_emails, user_id, user_email, ai_enabled, cache_duration, stats):
                        yield result
            ...
            # Yield final pipeline stats
            async for result in self._generate_final_stats(stats):
                yield result
    ...
    async def _setup_streaming_context(self, command: AnalysisCommand) -> Tuple[int, str, tzinfo, bool, int]:
        """Set up the user context for streaming email analysis.
        
        Args:
            command: The analysis command with parameters
            
        Returns:
            Tuple containing user_id, user_email, timezone_obj, ai_enabled, and cache_duration
    ...
    async def _send_cache_status(self) -> AsyncGenerator[Dict, None]:
        """Send a status update that we're checking the cache.
    ...
    async def _fetch_cached_emails(self, command: AnalysisCommand, user_email: str, user_timezone: tzinfo, stats: Dict) -> Tuple[List[ProcessedEmail], Set[str]]:
        ...
        Returns:
            Tuple of (cached_emails, cached_ids)
        """
        cached_emails = []
        cached_ids = set()
        ...
        if self.cache:
            try:
                ...
                timezone_str = str(user_timezone) if hasattr(user_timezone, 'key') else 'UTC'
                cached_emails = await self.cache.get_recent(command.cache_duration_days, command.days_back, user_email, timezone_str)
    ...
    async def _fetch_streaming_emails(self, command: AnalysisCommand, user_email: str, user_timezone: tzinfo, 
                                     cached_ids: Set[str], cached_emails: List[ProcessedEmail], 
                                     stats: Dict) -> Tuple[List[Dict], Set[str], List[Dict]]:
        """Fetch emails from Gmail for streaming.
        ...
        Returns:
            Tuple of (raw_emails, gmail_email_ids, new_raw_emails)
        """
        # Connect to Gmail
        await self.connection.connect(user_email)
        
        # Convert timezone object to string if needed
        timezone_str = user_timezone.key if hasattr(user_timezone, 'key') else 'UTC'
    ...
    def _filter_cached_emails(self, cached_emails: List[ProcessedEmail], gmail_email_ids: Set[str], stats: Dict) -> Tuple[List[ProcessedEmail], Set[str]]:
        """Filter cached emails to keep only those still in Gmail.
        ...
        Returns:
            Tuple of (filtered_cached_emails, filtered_cached_ids)
        """
    ...
    async def _process_without_ai(self, parsed_emails: List[EmailMetadata], user_email: str, cache_duration: int, stats: Dict) -> AsyncGenerator[Dict, None]:
    ...
    async def _process_in_batches(self, parsed_emails: List[EmailMetadata], command: AnalysisCommand, 
                                 user_id: int, user_email: str, ai_enabled: bool, 
                                 cache_duration: int, stats: Dict) -> AsyncGenerator[Dict, None]:
        ...
        for i in range(0, len(parsed_emails), command.batch_size):
            ...
            async for result in self._process_batch_results(batch_results, i, command.batch_size, len(parsed_emails), stats):
                yield result

    async def _process_batch_results(self, batch_results: List[ProcessedEmail], 
                                    batch_start_index: int, batch_size: int, 
                                    total_emails: int, stats: Dict) -> AsyncGenerator[Dict, None]:
        """Process and yield batch results.
    ...
    async def _process_all_at_once(self, parsed_emails: List[EmailMetadata], user_id: int, 
                                  user_email: str, ai_enabled: bool, 
                                  cache_duration: int, stats: Dict) -> AsyncGenerator[Dict, None]:
    ...
    async def _generate_final_stats(self, stats: Dict) -> AsyncGenerator[Dict, None]:
        ...
        # Yield detailed final stats as a dictionary
        yield {
            'type': 'stats',
            'data': {
